...
\begin{frame}[label=wwhistory]{WeBWorK history}
  ...
  Here's a possible starting point for the timeline- feel free 
  to delete it if you already have something :)
  \centering
  \begin{tikzpicture}
       ...
       %draw hours
       \foreach \x/\mydescription  in {0/Fall 2011,5/Aug 2012,8/Fall 2012} {% 
            \draw (\x,0) node[below=7pt,anchor=east,xshift=0,rotate=45](\mydescription) {\mydescription}; 
            \draw[] (\x,-0.3) -- (\x,0.3);
            }
            \node[below=of Fall 2011,align=center](PCC){PCC's big push};
            \pause
            \node[below=of PCC,align=center](SAC){Dept. meets and\\ forms committee};
            \pause
            \node[below=of SAC,align=center]{Initial experiments};
            \pause
            %\node[below=of May 2012,align=center]{Further talks};
            \node[below=of Aug 2012,align=center](release){Release time \\ approved};
            \pause
            \node[below=of release,align=center]{One or two instructors?};
            \pause
            \node[below=of Fall 2012]{Project begins};
   \end{tikzpicture}
